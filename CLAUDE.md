...
- **MVP は受け取り専用**。送金 / HUD / Artifact 投稿は MVP 範囲外（2026-05-06 確定）。これらの UI を「ついでに足す」提案はしない。
- **OTP 入力で自動 submit しない**。6 桁完了 / IME Done でのフォーム送信は禁止（誤入力リカバリを潰すため）。CTA タップで明示的に送信する。
- 設定画面・アカウント情報編集・パスワード変更・通知許可・取引履歴・ホーム残高は実装済み。これらに変更を入れるときは Android / iOS 両方を必ず触る。
